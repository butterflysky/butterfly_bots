...
def openai_complete(prompt: str, stops: list[str], strip=True):
    try:
        ...
        if response["choices"][0]["text"]:
            ...
            return f"openai response didn't include answer:\n\n{response}"
...
async def raw_openai(ctx, prompt, *stops: str):
    ...
    await ctx.send(openai_complete(prompt, list(stops)))
...
async def story(ctx, *words: str):
    ...
    await ctx.send(openai_complete(prompt, [" Your story:"]))
...
async def tarot(ctx, *words: str):
    ...
    await ctx.send(openai_complete(prompt, [" Your reading:"]))
...
async def code(ctx, language: str, *words: str):
    ...
    await ctx.send(openai_complete(prompt, [" Your code:"], strip=False))
...
async def chat(ctx, *words: str):
    ...
    try:
        ...
        if response["choices"][0]["text"]:
            ...
            await ctx.send("<error> something went wrong getting an answer from openai")
    except Exception as e:
